...
get.het.seq <- function(ntax = 36, backbone.ntax = 10, trlen = 3, backbone.trlen = 1.5, slen = 1000){
    tr1 <- rtree(backbone.ntax)
    tr1$edge.length <- rexp(length(tr1$edge.length))
    tr1$edge.length <- tr1$edge.length/sum(tr1$edge.length) * backbone.trlen
    s1 <- get.seq(tr1, model = "gtr.g", l = slen)
    
    tr2 <- rtree(ntax - backbone.ntax)
    tr2$edge.length <- rexp(length(tr2$edge.length))
    tr2$edge.length <- tr2$edge.length/sum(tr2$edge.length) * (trlen - backbone.trlen)
    s2 <- get.seq(tr2, model = "gtr.g", l = slen, rootseq =  as.character(as.DNAbin(s1)[1, ]))
